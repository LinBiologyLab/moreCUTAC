...
stepdir=${workdir}/06.removealldup
...
gatk  --java-options "-Xmx500G" MarkDuplicatesSpark  -I ${inputdir1}/${runID}.bam -O ${stepdir}/${runID}.bam --remove-all-duplicates --create-output-bam-index true  --tmp-dir ${stepdir}  > ${outfile} 2> ${errfile}

module purge
ml SAMtools/1.14-GCC-11.2.0

samtools view -c -f 3  ${stepdir}/${runID}.bam  > ${stepdir}/${runID}.count.txt 2>> ${errfile}
